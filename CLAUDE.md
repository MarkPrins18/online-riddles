...
9. Sessie kwijt = kamer kwijt — anonieme auth betekent dat een andere browser/tab of cache clear de rol/score niet terug te claimen is; alleen opnieuw joinen als nieuwe speler. Voor community-bijdragen (packs/raadsels) is dit sinds kort optioneel te verhelpen: via de "Account"-knop in `CommunityNav` of de dismissible banner op `/community/mijn` (`components/account/`) kan een anonieme sessie vrijwillig geüpgraded worden naar een permanent account (e-mail+wachtwoord of een code per e-mail, via `lib/supabase/accountAuth.ts`), met behoud van dezelfde `auth.uid()` en dus van bestaande `created_by`-rijen — nergens verplicht. Voor kamers/spelscores (zie punt 1) blijft dit probleem onveranderd: die zijn nog steeds puur sessiegebonden.
